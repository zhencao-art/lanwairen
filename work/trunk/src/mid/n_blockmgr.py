...
import configuredb,n_phydisk,n_lvmmgr,n_raidmgr
import res_manager
...
def block_create_vg(vg_name,phy_devices):
    ...
    if configuredb.db_vg_check(vg_name,db_handle):
        msg = 'input vg_name is exists in db'
        logging.error(msg)
        return (BlockOpError.EINVAL,msg)
...
def block_remove_vg(vg_name):
    ...
    if not vg_config:
        ...
        logging.error(msg)
        return (BlockOpError.EINVAL,msg)
    if not n_lvmmgr.vg_remove_check(vg_name):
...
def block_pv_list():
    try:
        pv_objs = n_lvmmgr.pv_list()
    except Exception as e:
        return (BlockOpError.EVGOP,None)
    return (BlockOpError.SUCCESS,pv_objs)

def block_list_vg_pv(vg_name,pv_objs):
    try:
        ret = n_lvmmgr.vg_pv_list(vg_name,pv_objs)
    except Exception as e:
        return (BlockOpError.EVGOP,None)
    return (BlockOpError.SUCCESS,ret)

def block_lv_list():
    try:
        lv_objs = n_lvmmgr.lv_list()
    except Exception as e:
        return (BlockOpError.EVGOP,None)
    return (BlockOpError.SUCCESS,lv_objs)

def block_list_vg_lv(vg_name,lv_objs):
    try:
        ret = n_lvmmgr.vg_lv_list(vg_name,lv_objs)
    except Exception as e:
        return (BlockOpError.EVGOP,None)
    return (BlockOpError.SUCCESS,ret)
...
def block_extend_vg(vg_name,phy_devices):
    ...
    if not configuredb.db_vg_check(vg_name,db_handle):
        msg = 'input vg_name is not found in db'
        logging.error(msg)
        return (BlockOpError.EINVAL,msg)
...
def block_create_lv(res_handle,vg_name,lv_name,lv_size,lv_size_unit):
    try:
        if not vg_name in n_lvmmgr.vg_name_list():
            raise Exception('Volume Group %s is not found' % vg_name)
...
def block_list_lv():
    ...
    ret_objs = []
    for i in lv_objs:
        if not configuredb.db_vg_check(i.vg_name,db_handle):
            continue
        else:
            ret_objs.append(i)
...
def block_extend_lv(vg_name,lv_name,size,size_unit):
    try:
        lv_objs = n_lvmmgr.lv_extend(vg_name,lv_name,size,size_unit)
    except Exception as e:
        return  BlockOpError.ELVOP,str(e)

def block_reduce_lv(vg_name,lv_name,size,size_unit):
    try:
        lv_objs = n_lvmmgr.lv_reduce(vg_name,lv_name,size,size_unit)
    except Exception as e:
        return  BlockOpError.ELVOP,str(e)
